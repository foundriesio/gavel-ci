...
class CustomLogFormatterMixin:
    """
    A common mixin to customize json_logging formatters to include a context
    from `log_add_ctx`. It makes sure that the context values are propagated
    to all logs within the request context.
    """
...
class CustomJSONLogWebFormatter(
    CustomLogFormatterMixin, json_logging.JSONLogWebFormatter
):
    pass


class CustomJSONRequestLogFormatter(
    CustomLogFormatterMixin, json_logging.JSONRequestLogFormatter
):
    pass
...
def create_app(settings_object="gavel_ci.settings"):
    ...
    # This ensures json_logging is correctly configured each time the app is
    # created, addressing the issue with its use of global variables. init
    # loggers conditionally, just once, because json_logging goes mad otherwise
    if json_logging._current_framework is None:
        json_logging.init_flask(
            custom_formatter=CustomJSONLogWebFormatter, enable_json=True
        )
        json_logging.config_root_logger()
    # init instrumentation unconditionally, it is a different app every time.
    json_logging.init_request_instrument(
        app,
        custom_formatter=CustomJSONRequestLogFormatter,
        exclude_url_patterns=[r"/healthz"],
    )
...
def _monkey_patch_json_logging_request_logger():
    # Monkey-patch a memory leak in `json_logging` causing indefinite handlers
    # during app reloading. See:
    # https://github.com/bobbui/json-logging-python/blob/master/json_logging/__init__.py#L165
